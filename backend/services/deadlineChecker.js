...
const tracer = trace.getTracer("taskmanager-deadline-checker");
...
async function checkOverdueTasks() {
  return tracer.startActiveSpan("deadline_checker.cycle", async (span) => {
    try {
      ...
      if (!acquired) {
        ...
        span.addEvent("deadline_checker.lock_skipped");
        return;
      }

      span.addEvent("deadline_checker.lock_acquired");
      ...
      try {
        ...
        span.addEvent("deadline_checker.tasks_found", { count: overdueTasks.length });
        ...
        for (const task of overdueTasks) {
          try {
            ...
            task.alertSent = true;
            await task.save();
            span.addEvent("deadline_checker.alert_sent", {
              taskId: task.id,
              userId: task.User.id,
            });
            console.log(`📧 Alerte envoyée pour la tâche "${task.title}" (id: ${task.id})`);
          } catch (mailErr) {
            span.addEvent("deadline_checker.alert_failed", {
              taskId: task.id,
        ...
        span.addEvent("deadline_checker.lock_released");
      }
    } finally {
      span.end();
    }
